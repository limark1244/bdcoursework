...
def get_timetable():
    query = models.Timetable.select(models.Timetable.id, models.Timetable.date_, models.Timetable.time_, models.Teacher.name,
                                    models.Teacher.surname, models.Cabinet.number, models.Cabinet.building,
                                    models.Timetable.type) \
        .join(models.Cabinet, on=(models.Timetable.cabinet == models.Cabinet.id)) \
        .switch(models.Timetable) \
        .join(models.Teacher, on=(models.Timetable.teacher == models.Teacher.id) & (models.Timetable.available) & (models.Timetable.date_ >= datetime.today().strftime('%Y-%m-%d')))
...
def get_my_modules():
    query = models.Student_Module.select(models.Module.id, models.Module.name, models.Student_Module.quantity) \
        .join(models.Module,
...
def get_my_lessons():
    query = models.Timetable_Student_Module.select(models.Timetable.date_, models.Timetable.time_, models.Teacher.name,
                                            models.Teacher.surname, models.Cabinet.number, models.Cabinet.building,
                                            models.Timetable.type)\
    ...
    .switch(models.Timetable)\
    .join(models.Cabinet, on=(models.Timetable.cabinet == models.Cabinet.id)) \
    .join(models.Student_Module, on=((models.Timetable_Student_Module.student_module == models.Student_Module.id) & (models.Student_Module.student == STUDENT_ID)))
...
def get_cabinets():
    query = models.Cabinet.select()
    execute = list(query)
    return execute


def timetable_add(time_, date_, id_teacher, id_cabinet, type):
    j = {'available': True, 'time_': time_, 'date_': date_, 'teacher_id': id_teacher, 'id_cabinet': id_cabinet, 'type': type}
    Timetable.create(**j)
